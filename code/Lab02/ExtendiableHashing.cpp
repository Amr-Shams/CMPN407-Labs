	...
	else
	{
		for (int i = 0; i < RECORDSPERBUCKET; i++)
		{
			if (currentBucket.dataItem[i].valid == 0)
			{
				...
				currentBucket.dataItem[i].key = data.key;
				currentBucket.dataItem[i].data = data.data;
...
int insertItem(DataItem data, Bucket& currentBucket, GlobalDirectory& globaldirectory)
{
	...
	int value = insertItemIntoBucket(*globaldirectory.entry[hash], data);
	int count = 0;
	while (count < 5 && value == 0)
	{
		count++;
...
void searchItem(int key, Bucket& currentBucket, GlobalDirectory& globaldirectory)
{
	...
	findItemInBucket(*ourBucket, key);
}
...
int extendDirectory(GlobalDirectory& globaldirectory, int splitIndex)
{
	...
	// Split only the bucket
	int diff = globaldirectory.globalDepth - globaldirectory.entry[splitIndex]->localDepth;
	if (diff > 1)
	{
		Bucket* prevBucket = globaldirectory.entry[splitIndex];
		Bucket* bk1 = new Bucket(globaldirectory.entry[splitIndex]->localDepth+1);
		Bucket* bk2 = new Bucket(globaldirectory.entry[splitIndex]->localDepth+1);

		for (int i = splitIndex; i < splitIndex + pow(2, diff); i++)
		{
			globaldirectory.entry[i] = int((i - splitIndex) / pow(2, diff - 1)) == 0 ? bk1 : bk2;
		}
		for (int i = 0; i < 2; i++) {
			int hash = getCurrentHash(prevBucket->dataItem[i].key, globaldirectory.globalDepth);
			//insertItemIntoBucket(*globaldirectory.entry[hash], prevEntry[splitIndex]->dataItem[i]);
			insertItem(prevBucket->dataItem[i], *prevBucket, globaldirectory);
		}
		return 1;
	}

	// Split the entire table
	...
	for (int i = 0; i < globaldirectory.length; i++)
	{
		if (splitIndex == i / 2)
		{
			Bucket* bk1 = new Bucket(prevEntry[i / 2]->localDepth + 1);
			globaldirectory.entry[i] = bk1;

		}
		else
		{
			globaldirectory.entry[i] = prevEntry[i / 2];
		}
	}
	for (int i = 0; i < 2; i++) {
		int hash = getCurrentHash(prevEntry[splitIndex]->dataItem[i].key, globaldirectory.globalDepth);
		//insertItemIntoBucket(*globaldirectory.entry[hash], prevEntry[splitIndex]->dataItem[i]);
		insertItem(prevEntry[splitIndex]->dataItem[i], *prevEntry[0], globaldirectory);
	}
	return 1;
}
